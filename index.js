...
function getArtistByIndex(arr, idx) {
  let tempObj = arr[idx];
  return `the artist at index ` + tempObj.id + ` is ` + tempObj.name
}
...
// console.log(addArtist(artists, 'morgan', '1999 - 2022', 'web design', 'american', 'i like sushi'))
...
function artistByCountry(arr, nationality){
  let newArr = [];
  arr.map((artist) => {
    if(nationality === artist.nationality)
